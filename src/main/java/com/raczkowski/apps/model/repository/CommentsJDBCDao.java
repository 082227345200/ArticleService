...
public class CommentsJDBCDao implements CommentsDao {
    ...
    public void addComment(Comment comment, Article article) {
        ...
        try {
            ...
            preparedStatement.setInt(2, comment.getIdOfArticle());
    ...
    public List<Comment> commentsOfArticles(int idOfArticle) {
        ...
        try {
            ...
            while (rs.next()) {
                int id = rs.getInt("id");
                int idOfReadArticle = rs.getInt("idofarticle");
                String content = rs.getString("content");
                String author = rs.getString("author");
                Date localDate = rs.getDate("localdate");
                Comment comment = new Comment(id, idOfReadArticle, content, author, localDate.toLocalDate());
                comments.add(comment);
            }
            statement.close();
        } catch (SQLException e) {
